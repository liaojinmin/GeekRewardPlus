package me.geek.reward.kether.sub
...
object KetherAPI {
    ...
    fun eval(player: Player, script: String): CompletableFuture<Any?> {
        return mirrorNow("GeekRewardPlus:Script") {
            return@mirrorNow try {
                KetherShell.eval(script, namespace = listOf("GeekRewardPlus")) {
    ...
    fun instantKether(player: Player, script: String, timeout: Long = 100): EvalResult {
        return try {
            ...
            GeekRewardPlus.say("§8解析 kether Timeout")
            EvalResult.FALSE
        }
    }
